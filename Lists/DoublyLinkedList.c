...
void insert_front(List *list, int val) 
{
    ...
    new_node->data = val;
    new_node->prev = NULL;
    new_node->next = traverse; //insert at front
    ...
    if (traverse != NULL) 
	{
        traverse->prev = new_node; //the original node's prev points to new node
    }
...
void insert_end(List *list, int val) 
{
    ...
    if (traverse == NULL) 
	{
        ...
        list->head = new_node;  //new node is the first and last node
        return;
    }

    while (traverse->next != NULL) //traverse till the last node
	{
    ...
    traverse->next = new_node; //last nodes' next points to new node
    new_node->data = val;
    new_node->next = NULL;
	
    new_node->prev = traverse;//new_nodes prev points to last node and 
}   
...
void insert_pos(List *list, int pos, int val) 
{
    ...
    int counter = 1;  //similar logic as SLL
    ...
    while (traverse != NULL) 
	{
        ...
        if (pos == counter) 
		{
            ...
            if (traverse->next != NULL)  //if this isnt last node
			{
				//next nodes's prev should point to new_node
